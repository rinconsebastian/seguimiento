...
namespace seguimiento.Controllers
{
    public class IndicadorsController : Controller
    {
        ...
        public async Task<IActionResult> ListadoResponsable(int itemId)
        {
            var responsable = db.Responsable.Find(itemId);
            if(responsable != null)
            {
                @ViewBag.responsable = responsable.Nombre;
            }
            ...
            //-----------consulta los indicadores que pertenecen a esa categoria si no lo logra genera alerta
            indicadores = await db.Indicador.Where(n => n.Categoria.IdResponsable == idPadre).Include(n => n.TipoIndicador).ToListAsync();
        ...
        public async Task<Indicador> getFromId(int idindicador)
        {
            Indicador indicador = await db.Indicador.Where(n => n.id == idindicador).FirstOrDefaultAsync();
